...
timeout=60
counter=0
while ! docker-compose exec -T mongodb mongosh --eval "db.runCommand('ping').ok" >/dev/null 2>&1; do
    if [ $counter -ge $timeout ]; then
        log_error "MongoDB did not start within ${timeout} seconds"
...
log_info "  - Connect Client:  POST http://localhost:8080/connect-client"
log_info "  - Debug Info:      http://localhost:8080/debug/config"
log_info ""
log_info "  MongoDB:           mongodb://admin:password123@localhost:27017/tennex"
log_info "  Mongo Express:     http://localhost:8081 (admin:admin123)"
...
log_info "  curl -X POST http://localhost:8080/connect-client -H 'Content-Type: application/json' -d '{\"client_id\":\"test123\"}'"
