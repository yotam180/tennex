...
// Helper function to handle network errors consistently
function handleNetworkError(error: unknown, operation: string): Error {
  // Only treat actual network/connection errors as "backend offline"
  if (error instanceof TypeError && (error.message.includes('fetch') || error.message.includes('Failed to fetch') || error.message.includes('network'))) {
    return new Error(`Backend server is offline or unreachable at ${getBackendUrl()}. Please start the backend server and try again.`);
  }
  
  if (error instanceof Error && error.cause && (error.cause as any).code === 'ECONNREFUSED') {
    return new Error(`Cannot connect to backend server at ${getBackendUrl()}. Please ensure the backend is running.`);
  }
  
  // For all other errors (including HTTP errors like 400, 401, etc.), pass through the original message
  return error instanceof Error ? error : new Error(`${operation} failed: ${error}`);
}
...
export function registerIpcHandlers(db: ReturnType<typeof drizzle<typeof schema>>) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          throw new Error(errorJson.error || errorJson.message || `Login failed: ${response.status}`);
        } catch {
      ...
      throw handleNetworkError(error, 'Login');
    }
  });
  ...
  ipcMain.handle('auth:register', async (_, userData: {
    ...
    try {
      ...
      throw handleNetworkError(error, 'Registration');
    }
  });
  ...
  ipcMain.handle('auth:getQR', async (_, accountId: string) => {
    try {
      ...
      throw handleNetworkError(error, 'QR generation');
    }
  });
}
