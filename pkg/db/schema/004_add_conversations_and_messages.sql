...
CREATE TABLE conversations (
    ...
    external_conversation_id TEXT NOT NULL, -- WhatsApp conversation ID, Telegram chat ID, etc.
    integration_type TEXT NOT NULL CHECK (integration_type IN ('whatsapp', 'email', 'telegram', 'discord', 'slack')),
    conversation_type TEXT NOT NULL CHECK (conversation_type IN ('individual', 'group', 'broadcast', 'channel')),
...
CREATE TABLE conversation_participants (
    ...
    external_user_id TEXT NOT NULL, -- JID, Telegram user ID, etc.
    integration_type TEXT NOT NULL CHECK (integration_type IN ('whatsapp', 'email', 'telegram', 'discord', 'slack')),
    
    -- Participant info
    display_name TEXT,
    role TEXT NOT NULL DEFAULT 'member' CHECK (role IN ('member', 'admin', 'owner', 'moderator')),
    
    -- Status
    is_active BOOLEAN NOT NULL DEFAULT TRUE,
    joined_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    left_at TIMESTAMPTZ,
    added_by_external_id TEXT, -- Who added this participant
...
CREATE TABLE messages (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    conversation_id UUID NOT NULL REFERENCES conversations(id) ON DELETE CASCADE,
    external_message_id TEXT NOT NULL, -- WhatsApp message ID, Telegram message ID, etc.
    external_server_id TEXT, -- Optional server-side ID (some platforms have this)
    integration_type TEXT NOT NULL CHECK (integration_type IN ('whatsapp', 'email', 'telegram', 'discord', 'slack')),
    
    -- Message source
    sender_external_id TEXT NOT NULL, -- Sender's platform ID
    sender_display_name TEXT, -- Sender's name at time of message
    
    -- Message content
    message_type TEXT NOT NULL CHECK (message_type IN ('text', 'image', 'video', 'audio', 'document', 'location', 'contact', 'sticker', 'poll', 'reaction', 'system')),
    content TEXT, -- Text content or caption
    
    -- Message metadata
    timestamp TIMESTAMPTZ NOT NULL,
    edit_timestamp TIMESTAMPTZ, -- When message was last edited
    ...
    reply_to_external_id TEXT, -- Original message's external ID
...
CREATE INDEX idx_messages_unread ON messages (conversation_id, timestamp) WHERE delivery_status != 'read';
...
CREATE TABLE message_media (
    ...
    original_url TEXT,
    thumbnail_url TEXT,
    local_file_path TEXT, -- If downloaded locally
    ...
    CONSTRAINT message_media_duration_check CHECK (duration_seconds IS NULL OR duration_seconds > 0),
...
CREATE INDEX idx_message_media_download_status ON message_media (download_status) WHERE download_status != 'completed';

-- Contacts table: integration-specific contact storage
CREATE TABLE contacts (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_integration_id INTEGER NOT NULL REFERENCES user_integrations(id) ON DELETE CASCADE,
    external_contact_id TEXT NOT NULL, -- Contact's platform ID
    integration_type TEXT NOT NULL CHECK (integration_type IN ('whatsapp', 'email', 'telegram', 'discord', 'slack')),
    ...
    last_name TEXT,
    phone_number TEXT,
    username TEXT, -- Platform username if exists
