...
INSERT INTO integration_settings (
    user_integration_id, setting_key, setting_value
) VALUES (
    $1::int, $2::text, $3::jsonb
) ON CONFLICT (user_integration_id, setting_key) DO UPDATE SET
    setting_value = EXCLUDED.setting_value,
    updated_at = NOW()
RETURNING id, user_integration_id, setting_key, setting_value, created_at, updated_at;

-- name: GetIntegrationSetting :one
SELECT id, user_integration_id, setting_key, setting_value, created_at, updated_at
FROM integration_settings 
WHERE user_integration_id = $1::int AND setting_key = $2::text;
