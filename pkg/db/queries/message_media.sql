...
SET file_name = $2::text,
    file_size = $3::bigint,
    mime_type = $4::text,
    duration_seconds = $5::int,
    width = $6::int,
    height = $7::int,
    platform_metadata = $8::jsonb,
    updated_at = NOW()
WHERE id = $1::uuid;
-- name: UpdateDownloadStatus :exec
UPDATE message_media
SET download_status = $2::text,
    updated_at = NOW()
WHERE id = $1::uuid;
-- name: MarkAsDownloaded :exec
UPDATE message_media
SET download_status = 'completed',
    local_file_path = $2::text,
    downloaded_at = NOW(),
    updated_at = NOW()
WHERE id = $1::uuid;
